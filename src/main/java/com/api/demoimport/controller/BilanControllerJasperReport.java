...
public class BilanControllerJasperReport {
    ...
    public ResponseEntity getEsg(@RequestParam("dateBilan") String date, @RequestParam("companyName") String company_name) throws JRException {
        try {
            ByteArrayOutputStream reportstream = reportServiceImpl.exportEsg(date, company_name);
            HttpHeaders httpHeaders = new HttpHeaders();
            httpHeaders.setContentType(MediaType.APPLICATION_PDF);
            return new ResponseEntity(reportstream.toByteArray(), httpHeaders, HttpStatus.OK);
        } catch (RuntimeException e) {
    ...
    public ResponseEntity getDetailCPC(@RequestParam("dateBilan") String date, @RequestParam("companyName") String company_name) throws JRException {
        try {
            ByteArrayOutputStream reportstream = reportServiceImpl.exportDetailCPC(date, company_name);
            HttpHeaders httpHeaders = new HttpHeaders();
            httpHeaders.setContentType(MediaType.APPLICATION_PDF);
            return new ResponseEntity(reportstream.toByteArray(), httpHeaders, HttpStatus.OK);
        } catch (RuntimeException e) {
    ...
    public ResponseEntity getAllTable(@RequestParam("dateBilan") String date, @RequestParam("companyName") String company_name) throws JRException {
        try {
            ByteArrayOutputStream reportstream = reportServiceImpl.exportAlltable(date, company_name);
            HttpHeaders httpHeaders = new HttpHeaders();
            httpHeaders.setContentType(MediaType.APPLICATION_PDF);
            return new ResponseEntity(reportstream.toByteArray(), httpHeaders, HttpStatus.OK);
        } catch (RuntimeException e) {
            String message = "Failed to upload all tables " + e.getLocalizedMessage();
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ResponseMessage(message));
        }
